...
class SqlPermissionsRepository(
  ...
  private fun putUserPermissions(id: String, resources: Set<Resource>) {
    ...
    toStore.chunked(dynamicConfigService.getConfig(Int::class.java, "permissions-repository.sql.write-batch-size", 100)) { chunk ->
    ...
    try {
      toDelete.groupBy { it.type }
        .forEach { (type, ids) ->
          ids.chunked(dynamicConfigService.getConfig(Int::class.java, "sql.cache.write-batch-size", 100)) { chunk ->
